...
#
# Tags look like `v<Soft Serve version>-<release>`, which is what this repository
# has always published (v0.4.6-0 ... v0.12.2-0):
#
# - if defaults/main.yml points at a Soft Serve version that has never been
#   released, the release counter restarts at 0 (`v0.12.3-0`)
# - otherwise the counter is incremented (`v0.12.2-1`), but only if something
...
# Anchored on `soft_serve_version:` so that neither a commented-out example nor
# `soft_serve_container_image_tag`, which is derived from it and whose name the
# Renovate annotation's own pattern would also accept, can be mistaken for it.
version="$(sed -nE 's|^soft_serve_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Soft Serve version from $defaults_path"
	exit 1
fi

# Soft Serve's own version carries a leading `v`, and so do this repository's
# tags, so the prefix is stripped and put back rather than doubled. A future
# upstream change of convention therefore cannot produce a `vv…` tag.
